...
import { ManImage, LegacyImage } from "./images";

export const SimpleStories = () => (
  <section className="flex flex-col gap-16 lg:gap-32 justify-center items-center">
    <SimpleStoryItem title={"L'HOMME"} image={<ManImage />}>
      ...
      rôle essentiel dans l&apos;ascension de l&apos;entreprise. Il a hérité de
      cette petite parcelle de terre avec une vision audacieuse et un désir
...
const SimpleStoryItem = ({
  ...
  >
    ...
    <div className="flex flex-col gap-4 justify-center">
      ...
      <p className="font-light lg:max-w-[512px] self-center px-16 lg:px-0">
        {children}
      </p>
    </div>
  </div>
...
const TitleMobile = ({ children }: React.PropsWithChildren) => (
  <h3
    className={`for-mobile font-cinzel relative z-1 text-4xl font-bold flex-1 flex justify-center items-center text-white`}
  >
    {children}
  </h3>
);
...
const TitleDesktop = ({ children }: React.PropsWithChildren) => (
  <h3
    className={`for-desktop font-cinzel relative z-1 text-6xl flex items-center`}
  >
    {children}
  </h3>
);
